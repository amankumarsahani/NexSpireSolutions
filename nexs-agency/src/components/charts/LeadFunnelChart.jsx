    ...
    return (
        <div className="bg-white rounded-xl shadow-sm border p-6">
            ...
            <div className="grid grid-cols-3 gap-3 mt-4">
                {chartData.slice(1).map((item, index) => (
                    <div key={index} className="text-center p-3 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-xs text-green-600 font-medium">{getConversionRate(index + 1)} conversion</p>
                    </div>
                ))}
            </div>
            ...
            <div className="mt-4 pt-4 border-t text-center">
                ...
                <p className="text-xs text-gray-500 mt-1">{chartData[chartData.length - 1].value} of {chartData[0].value} leads converted</p>
            </div>
        </div>
    );
}
